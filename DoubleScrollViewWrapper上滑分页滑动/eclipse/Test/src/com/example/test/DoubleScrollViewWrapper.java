...
public class DoubleScrollViewWrapper extends ScrollView {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr) {
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.DoubleScrollViewWrapper);
		int padding = a
				.getInteger(
						R.styleable.DoubleScrollViewWrapper_doublescrollviewwrapper_padding,
						0);
		heitht = UiUtils.getScreenHeight(context)
				- UiUtils.getStatusBarHeight(context)
				- UiUtils.dp2px(context, padding);
		a.recycle();
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			int creteria = heitht / 5;// 滑动多少距离,然后才翻�?
			if (ispageOne) {
				if (scrollY <= creteria) {
					// 返回PageOne
					...
					if (pageTwo instanceof PageTwoWebView) {
						((PageTwoWebView) pageTwo)
								.loadUrl("https://github.com/ysnows");
