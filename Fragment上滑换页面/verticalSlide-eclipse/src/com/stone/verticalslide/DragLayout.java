...
/**
 * 这是�?个viewGroup容器，实现上下两个frameLayout拖动切换
 * 
...
public class DragLayout extends ViewGroup {
	...
	private static final int DISTANCE_THRESHOLD = 100; // 单位是像素，当上下滑动�?�度不够时，通过这个阈�?�来判定是应该粘到顶部还是底�?
	private int downTop1; // 手指按下的时候，frameView1的getTop�?
	...
	private class DragHelperCallback extends ViewDragHelper.Callback {
		...
		public int getViewVerticalDragRange(View child) {
			// 这个用来控制拖拽过程中松手后，自动滑行的速度，暂时给�?个随意的数�??
			return 1;
		}

		@Override
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			// 滑动松开后，�?要向上或者乡下粘到特定的位置
		...
		public int clampViewPositionVertical(View child, int top, int dy) {
			...
			// finalTop代表的是理论上应该拖动到的位置�?�此处计算拖动的距离除以�?个参�?(3)，是让滑动的速度变慢。数值越大，滑动的越�?
	...
	/* touch事件的拦截与处理都交给mDraghelper来处�? */
	@Override
	public boolean onInterceptTouchEvent(MotionEvent ev) {

		if (frameView1.getBottom() > 0 && frameView1.getTop() < 0) {
			// view粘到顶部或底部，正在动画中的时�?�，不处理touch事件
			return false;
		}
		...
		if (action == MotionEvent.ACTION_DOWN) {
			// action_down时就让mDragHelper�?始工作，否则有时候导致异�? 他大爷的
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (frameView1.getTop() == 0) {
			...
			// 如果已被初始化，这次onLayout只需要将之前的状态存入即�?
	...
	/**
	 * 这是View的方法，该方法不支持android低版本（2.2�?2.3）的操作系统，所以手动复制过来以免强制�??�?
	 */
